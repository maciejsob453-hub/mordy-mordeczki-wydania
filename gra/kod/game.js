...
const INNATE={
 ...
 'Vengeance':{n:'Człowiek Piła',c:'#c0392b',
   d:'Pretensjonalność +1,6 i sława +1,8 tygodniowo, bo lider napierdala po japońsku i nikt nie wie o co chodzi, ale wszyscy słuchają. Autorytet rośnie o 5 z każdą kadencją, aż w końcu wyjdzie z tego ta postać.'},
...
function enGain(){
  const p=me();
  return Math.max(0, Math.round((BAL.energiaBaza+lead(G.me).wytrz/3.1+(p.uni-42)/4.4)*10)/10);
}
...
function shown(k,pct){ // wartość pokazywana w sondażu, nie prawdziwa
  return Math.max(0,pct+((G.noise&&G.noise[k])||0));
}
...
function newGame(id){
  ...
  G={me:id,p,rel,coal:startCoal,term:1,week:1,weeks:12,ap:3,apMax:3,kp:26,en:100,
     ...
     turnout:.85,lup:{},recCd:0,xp:0,traits:[],ptraits:{},tut:null,tutSeen:{},streak:0,noise:{},useTerm:{},catUsed:{},lastAct:null,
...
function XP(n){if(G)G.xp=(G.xp||0)+n}
function checkDeath(){
...
function lists(res,total){
  // najpierw próg indywidualny: partia poniżej 5% nie bierze mandatów, także wewnątrz bloku
  const elig={},pctOf={};
  alive().forEach(k=>{pctOf[k]=res[k].tot/total*100; elig[k]=pctOf[k]>=THR.base});
  const Ls=[],done=new Set();
  for(const c in CO()){
    const all=CO()[c].m.filter(k=>!G.p[k].dead);
    if(all.length<2)continue;
    all.forEach(k=>done.add(k));
    const blocPct=all.reduce((a,k)=>a+res[k].tot,0)/total*100;
    const m=all.filter(k=>elig[k]);                       // tylko ci, którzy sami mają 5%
    Ls.push({id:c,coal:true,m,all,n:all.length,
      tot:m.reduce((a,k)=>a+res[k].tot,0),blocPct,
      reg:Object.fromEntries(REG.map(r=>[r.id,m.reduce((a,k)=>a+res[k].reg[r.id],0)]))});
  }
  ...
  Ls.forEach(l=>{
    ...
    l.thr=thrFor(l.n);
    l.in = l.blocPct>=l.thr && l.m.length>0;                // blok musi mieć próg I kogoś powyżej 5%
  });
  return Ls;
}
...
{id:'wiec',cat:'kam',n:'Wiec w kanale',ap:1,kp:6,en:8,reg:1,tem:1,
 ...
 f:(p,f,_,r,__,t)=>{
   ...
   if(roll>.85){M(p,8);p.pres[r]=cl(p.pres[r]+46*f);p.fame=cl(p.fame+R(5,8)*f);p.uni=cl(p.uni+3);p.act=cl(p.act+4);
...
{id:'szkol',cat:'org',n:'Szkolenie kadr',ap:1,kp:13,en:7,
 ...
 f:(p,f)=>{p.act=cl(p.act+R(6,10)*f);p.uni=cl(p.uni+4*f);p.cred=cl(p.cred+2);p.ctr=cl(p.ctr-3);
...
{id:'statut',cat:'org',n:'Reforma statutu',ap:2,kp:16,en:11,
 ...
 f:(p,f)=>{if(p.uni<40&&ch(.42)){const q=giveBackCap(p,Math.max(1,Math.round(p.mem*.16)));const l=q.eli+q.int+q.ser;p.uni=cl(p.uni-6);
  ...
  const g=R(8,13)*f;p.uni=cl(p.uni+g);p.cred=cl(p.cred+4*f);return `Statut przyjęty. <b>Jedność +${Math.round(g)}</b>.`}},
{id:'czyst',cat:'org',n:'Czystka w partii',ap:2,kp:0,en:16,
 ...
 f:(p,f)=>{const q=giveBackCap(p,Math.max(1,Math.round(p.mem*R(.10,.22))));const l=q.eli+q.int+q.ser;
  p.uni=cl(p.uni+R(14,22));p.ctr=cl(p.ctr+10);p.cred=cl(p.cred-5);
...
{id:'zjazd',cat:'org',n:'Zjazd partii',ap:2,kp:105,en:26,term1:1,
 ...
 f:(p,f)=>{M(p,22);p.fame=cl(p.fame+R(12,19)*f);p.uni=cl(p.uni+16*f);p.act=cl(p.act+18*f);p.cred=cl(p.cred+8);
...
{id:'podkup',cat:'bru',n:'Podkupienie działacza',ap:1,kp:20,en:9,tgt:1,
 d:'Pula 670 osób jest skończona, czasem trzeba wziąć od kogoś.',
 f:(p,f,t)=>{const o=G.p[t];
  if(ch(.30)){p.cred=cl(p.cred-9);p.ctr=cl(p.ctr+14);
   G.rel[G.me][t]=cl(G.rel[G.me][t]-32,-100,100);G.rel[t][G.me]=cl(G.rel[t][G.me]-32,-100,100);
   return `<b>Wyciekły DM-y.</b> Wszyscy wiedzą, że kupowałeś ludzi.`}
  const g=Math.min(o.mem-1,RI(1,2));if(g<1)return `${o.ab} nie ma już kogo oddać.`;
  for(let i=0;i<g;i++){const gr=o.comp.eli>0&&ch(.3)?'eli':o.comp.int>0?'int':'ser';
    if(o.comp[gr]<1)continue;o.comp[gr]--;o.mem--;p.comp[gr]++;p.mem++}G.rel[G.me][t]=cl(G.rel[G.me][t]-18,-100,100);G.rel[t][G.me]=cl(G.rel[t][G.me]-18,-100,100);
  return `<b>${g}</b> od ${o.ab} przechodzi do ciebie.`}},
/* --- brudne --- */
...
{id:'dymisja',cat:'prem',n:'Odwołanie ministra',ap:2,kp:12,en:12,pm:1,
 d:'Wyrzucasz partię z rządu. Sejm głosuje, i nie zawsze jest po twojej stronie.',
...
{id:'kanaly',cat:'prem',n:'Ustawa o kanałach',ap:2,kp:28,en:15,pm:1,term1:1,
 d:'Tworzysz nowy kanał albo likwidujesz istniejący. Mandaty okręgowe rozdzielają się wtedy od nowa, więc przewraca to całą mapę wyborczą. Raz na kadencję.',
 f:()=>{openKanaly();return null}},
/* --- opozycja --- */
...
{id:'odp',cat:'spe',n:'Regeneracja lidera',ap:1,kp:0,en:-36,
 d:'Lider znika na tydzień. Wraca z energią.',
...
function game(){
  ...
  <div class="nav">
    ${(()=>{const nv=[['mapa','Mapa okręgów'],['akcje','Decyzje'+(G.ap?`<span class="badge">${G.ap}</span>`:'')],
       ['lider','Lider'+(G.xp>=35?'<span class="badge">!</span>':'')],['krol','Król'+(kingFav(G.me)<0?'<span class="badge">!</span>':'')],['sondaz','Sondaż']];
      const mg=myGoals();
...
const AFX={
 ...
 dymisja:['rel','fame','risk'], rozwiaz:['risk','ctr'],
...
function actCards(list,fx){
  return list.map(a=>{
    ...
    const kpC=Math.round(a.kp*sizeF(me()).kp);
    ...
    const blok=done?'wykorzystane':usedT?'zużyte w tej kadencji'
      :catFull?`${katN}: decyzja z tej kategorii zużyta — wraca za 1 tydzień`
...
function premierTab(){
  ...
  <div class="card urzad prem" style="margin-top:14px"><div class="h"><h3>Rada ministrów</h3>
    ...
    <div class="resgrid">${RESORTY.map(r=>{
      ...
      return `<button class="resort ${kto?'on':''} ${swoj?'swoj':''}" onclick="openResort('${r.id}')">
...
function actTab(){
  ...
  return `<div class="card"><div class="h"><h3>Decyzje tygodnia</h3>
    <span class="n">${ikona('akcje','sm')}${G.ap}/${G.apMax} akcji · ${ikona('kapital','sm')}${Math.round(G.kp)} kapitału · ${ikona('energia','sm')}${Math.round(G.en)} energii</span></div>
    <div class="b">
...
function chooseTem(){
  ...
  modal('Temat wystąpienia',pend.a.n,
    ...
    TEM.map(t=>{
      let hint='';
      if(r){let m=0;SID.forEach(s=>m+=r.mix[s]*(t.w[s]||0));
        hint=m>.9?'<span class="ok">świetnie pasuje</span>':m>.35?'pasuje przyzwoicie':m>-.1?'obojętne':'<span class="bad">wrogie temu kanałowi</span>'}
      else hint=SID.filter(s=>(t.w[s]||0)>=1.5).map(sn).join(', ');
      return {l:t.n,s:hint,f:()=>{pend.tem=t.id;close();step()}}}))}
function chooseTgt(){
...
function naborPublikuj(){
    ...
    G.recCd=hasT('sieciowiec')?2:(p.mem<12?2:3);
...
function openKanaly(){
  G.lastCharge=null;
  const canAdd=REG.length<12, canDel=REG.length>5;
  const opts=[];
  if(canAdd)opts.push({l:'Tworzę nowy kanał',s:`Obecnie ${REG.length} okręgów, maksymalnie 12`,
    f:()=>{close();modalNewKanal()}});
  if(canDel)REG.slice().sort((a,b)=>a.pop-b.pop).slice(0,4).forEach(r=>{
    opts.push({l:`Likwiduję ${r.n}`,s:`${r.pop} osób, ${r.seats} ${pl(r.seats,'mandat','mandaty','mandatów')}, ludzie rozejdą się po pozostałych`,
      f:()=>{delRegion(r.id);close();render()}})});
  opts.push({l:'Zostawiam jak jest',s:'',f:close});
  modal('Ustawa o kanałach',`Struktura okręgów (${REG.length}/12)`,
    `<p>Możesz utworzyć nowy kanał albo zlikwidować istniejący. Mandaty okręgowe rozdzielają się na nowo
     proporcjonalnie do liczby głosujących, minimum dwa na okręg.</p>`,opts);
}
function modalNewKanal(){
  G.lastCharge=null;
  close();
  const v=document.createElement('div');v.className='veil';v.id='veil';
  v.innerHTML=`<div class="mdl"><button class="mdlx" type="button" aria-label="Zamknij">×</button>
    <div class="h"><div class="k">Ustawa o kanałach</div><h2>Nowy okręg</h2></div>
    <div class="bd"><p>Podaj nazwę kanału. Hasztag dopisze się sam.</p>
      <input class="inp" id="kn" maxlength="24" placeholder="np. archiwum">
      <p style="font-size:12.5px;color:var(--dim2);margin:6px 0 0">Nowy kanał startuje jako mieszany:
      około 8% elity, 26% intelektualistów, 66% serwerowiczów. Ludzie przyjdą z pozostałych kanałów.</p></div>
    <div class="op"><button class="opt" id="ok"><b>Zakładam kanał</b><span></span></button>
      <button class="opt" id="no"><b>Rezygnuję</b><span></span></button></div></div>`;
  document.body.appendChild(v);
  v.querySelector('#no').onclick=()=>{close();render()};
  v.querySelector('.mdlx').onclick=()=>{close();render()};
  v.querySelector('#ok').onclick=()=>{
    let n=(v.querySelector('#kn').value||'nowy').trim().replace(/^#+/,'').slice(0,24)||'nowy';
    addRegion('#'+n);close();render();
  };
}
function addRegion(name){
  const pop=Math.max(30,Math.round(REG.reduce((a,r)=>a+r.pop,0)/REG.length*.55));
  REG.forEach(r=>{const t=Math.round(pop/REG.length);r.pop=Math.max(25,r.pop-t)});
  const id='r'+Date.now().toString(36);
  const row=Math.floor(REG.length/3), col=REG.length%3;
  REG.push({id,n:name,pop,eng:.34,seats:2,x:250+col*199,y:135+row*172.5,
    mix:{eli:.08,int:.26,ser:.66},d:'Kanał powołany ustawą. Skład mieszany, frekwencja przeciętna.'});
  PID.forEach(k=>G.p[k].pres[id]=RI(2,10));
  rebalanceSeats();
  say(`<b>Powstaje ${name}.</b> Okręgów: ${REG.length}, mandaty rozdzielone na nowo.`,'roy');
}
function delRegion(id){
  const i=REG.findIndex(r=>r.id===id);if(i<0||REG.length<=5)return;
  const r=REG[i];const share=Math.round(r.pop/(REG.length-1));
  REG.splice(i,1);REG.forEach(x=>x.pop+=share);
  PID.forEach(k=>delete G.p[k].pres[id]);
  if(G.sel===id)G.sel=REG[0].id;
  rebalanceSeats();
  say(`<b>${r.n} zlikwidowany.</b> Okręgów: ${REG.length}.`,'roy');
}
...
const radaInit=()=>{if(!G.rada)G.rada={}};
...
function obsadz(id,nick,zPartii){
  ...
  if(stary&&stary!==nick){const kS=partiaOsoby(stary);
    if(kS&&kS!==G.me){G.rel[G.me][kS]=cl(G.rel[G.me][kS]-6,-100,100);G.rel[kS][G.me]=cl(G.rel[kS][G.me]-6,-100,100)}}
  if(nick){
    G.rada[id]=nick;
    if(zPartii){
...
function openDym(){
  ...
  modal('Premier','Odwołanie ministra',
    `<p>Wniosek trafia pod głosowanie sejmu. Twoi koalicjanci mogą stanąć po stronie odwoływanego.</p>`,
    cand.map(k=>({l:`Odwołuję ${G.p[k].lead} (${G.p[k].ab})`,
      s:`${ministrowieZ(k).map(r=>r.n).join(', ')} · relacje ${Math.round(G.rel[G.me][k])}`,
      f:()=>{close();
        const v=sejmVote('minister',k,G.me,1);
        const resorty=ministrowieZ(k).map(r=>r.n).join(', ');
        if(v.pass){govLeave(k);G.rel[G.me][k]=-55;G.rel[k][G.me]=-55;me().fame=cl(me().fame+6);me().act=cl(me().act+7);me().ctr=cl(me().ctr+12);
          RESORTY.forEach(r=>{const n=radaKto(r.id);if(n&&partiaOsoby(n)===k)delete G.rada[r.id]});
          say(`<b>Sejm odwołał ${G.p[k].lead}</b> ${v.yes}:${v.no}. ${G.p[k].ab} poza rządem.`,'bad');
          modal('Sejm','Minister odwołany',
            `<p>Za <b>${v.yes}</b>, przeciw <b>${v.no}</b>, wstrzymało się <b>${v.abst}</b>.</p>${glosyBox(v)}
             <p style="margin-top:12px"><b>${G.p[k].ab}</b> wypada z rządu, a ${resorty||'jego resorty'} ${resorty.includes(',')?'wracają':'wraca'} do obsadzenia.</p>`,
            [{l:'Rozumiem',f:()=>{close();render()}}]);}
        else{APPR(-6);G.rel[G.me][k]=cl(G.rel[G.me][k]-20,-100,100);
          say(`<b>Sejm odrzucił odwołanie</b> ${v.yes}:${v.no}. Wyszedłeś na słabego.`,'bad');
          modal('Sejm','Odwołanie odrzucone',
            `<p>Za <b>${v.yes}</b>, przeciw <b>${v.no}</b>, wstrzymało się <b>${v.abst}</b>.</p>${glosyBox(v)}
             <p style="margin-top:12px">Koalicjanci stanęli po stronie odwoływanego. Poparcie rządu w dół, relacje z ${G.p[k].ab} też.</p>`,
            [{l:'Rozumiem',f:()=>{close();render()}}]);}
        render()}}))
...
const LAWPAR={
 ...
 ordyn:{
  ...
  zakres:{prog:[0,15],mandaty:[20,60]},
...
function signLaw(ok,cicho){
  ...
  if(ok){
    ...
    if(pmK&&G.p[pmK]){
      const q=G.p[pmK];
      q.comp.ser++;q.mem++;q.act=cl(q.act+1);q.fame=cl(q.fame+2);
...
const GOALS={
 republika:{n:'Pod błyskiem niebieskiej chwały',for:['PPP','PLR','PKD','DPD'],logo:'REP',bots:0,
  ...
  req:[
   ...
   {t:'Kaziu, Europejczyk, Eniki i Ponczus w partii',
    v:()=>{const o=roster(me());return ['Kaziu','Europejczyk','Eniki','Ponczus'].filter(n=>o.includes(n)).length+' / 4'},
    ok:()=>{const o=roster(me());return ['Kaziu','Europejczyk','Eniki','Ponczus'].every(n=>o.includes(n))}},
 ...
 ads:{n:'Alternatywa dla nowości',for:['DPD','FD','PLR'],logo:'ADS',bots:1,
  ...
  req:[
   ...
   {t:'loof, Tortex i Kaziu w składzie partii',v:()=>{const o=ownPool(G.me);return ['loof','Tortex','Kaziu'].filter(n=>o.includes(n)).length+' / 3'},
    ok:()=>{const o=ownPool(G.me);return ['loof','Tortex','Kaziu'].every(n=>o.includes(n))}},
...
function leadTab(){
  ...
  return `<div class="card"><div class="h"><h3>${leads(p).join(' i ')}, rozwój</h3>
    <span class="n">doświadczenie: <b style="color:var(--acc)">${G.xp}</b></span></div><div class="b">
    ...
    fotel premiera i prezydenturę. Wydajesz je tutaj, i tylko tutaj.</p>
    ${wielu?`<div class="sterlab" style="margin-top:18px">Kogo rozwijasz</div>
      ...
      <div class="note" style="margin:0 0 12px">Doświadczenie jest wspólne dla partii, ale kupujesz je
      konkretnej osobie. Cecha kupiona komukolwiek ze sterów działa na całą partię, tyle że
      za każdą płacisz osobno.</div>`:''}
    ...
    ${TRAITS.map(t=>{const has=traitsOf(kto).includes(t.id),can=G.xp>=t.cost,cf=conflictOf(t,kto);
    ...
    ${STAT.map(([id,n,d],i)=>{const v=L(kto)[id],c=cost(v);
      return `<div style="display:flex;align-items:center;gap:12px;margin-bottom:10px">
        ...
        <button class="btn ${G.xp>=c&&v<99?'':'g'} sm" ${G.xp>=c&&v<99?'':'disabled'}
...
function buyTrait(id){
  ...
  if(!t||!who||traitsOf(who).includes(id)||G.xp<t.cost||conflictOf(t,who))return;
  if(!G.ptraits)G.ptraits={};
  if(!G.ptraits[who])G.ptraits[who]=[];
  G.xp-=t.cost;G.ptraits[who].push(id);
...
function buyStat(i){
  ...
  if(G.xp<c||v>=99)return;
  G.xp-=c;if(!G.lup[who])G.lup[who]=[0,0,0,0];G.lup[who][i]+=1;render();
}
...
function startTerm(){
  ...
  G.apMax=apBase();G.ap=G.apMax;
  if(alive().some(k=>isLead(G.p[k],'Vengeance'))&&LEAD['Vengeance'][3]<99){
    LEAD['Vengeance'][3]=Math.min(99,LEAD['Vengeance'][3]+5);
  }
...
window.__game={openDym,mojeResorty,mogeZglosic,rozwiazChance,radaKto,RESORTY,pmOsoba,pmOsoby,leads,roster,
  ...
  giveBackCap,prezRound1,prezRound2,runRunoff,memberFlow,prezWait,prezPush,openPush,crownPrez,hemi,pmBlocked,rotateBench,AVA,TEM,INNATE,conflictOf,buyTrait,traitsOf,COMBO,checkDeath,isPMperson,isPrezPerson,income,EV,wotumChance,
