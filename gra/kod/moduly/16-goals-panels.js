...
[ ['nplr_liberal','obrazki/cel-liberalizm.svg'],['nplr_hotdog','obrazki/16ca0d3b9eeb.webp'],['nplr_rose','obrazki/logo-partia-liberalna.png'],['nplr_king','obrazki/0051a52917ef.webp'],['nplr_jewish','obrazki/cel-gwiazda.svg'],['nplr_star','obrazki/logo-alternatywa-zydowska.png'],['nplr_blue','obrazki/cel-oko.svg'],['nplr_diplomacy','obrazki/cel-dyplomacja.svg'],['nplr_council','obrazki/cel-rada.svg'],['nplr_gloria','obrazki/a56a5574593a.webp'],['nplr_power','obrazki/cel-rada.svg'],['nplr_friendship','obrazki/cel-dyplomacja.svg'],['nplr_aurea','obrazki/logo-aurea-libertas.png'] ].forEach(([id,avatar])=>{if(NATIONAL_GOALS[id])NATIONAL_GOALS[id].avatar=avatar});
...
function goalOpen(id){
  ...
  G.goalFocus=id;
  const g=item.g,done=item.done;
  const req=item.type==='party'?reqOf(id):[];
  const conditions=item.type==='party'
    ?req.map(r=>`${r.ok()||done?'✓':'×'} ${typeof r.t==='function'?r.t():r.t}`).join('<br>')
    :(done?'Cel wykonany.':item.reason||g.accessText||'Poprzedni cel musi być ukończony.');
  const effects=(g.cons||[]).map(x=>`<li>${x}</li>`).join('');
  const buttons=[];
  if(item.type==='party'&&!done)buttons.push({l:item.ready?'Wypełniam cel':'Warunki niespełnione',dis:!item.ready,f:()=>{close();if(item.ready)doGoal(id)}});
  if(item.type==='national'&&!done&&!item.active&&item.ready){
    if(g.optional&&!item.opted)buttons.push({l:'Podejmuję cel',f:()=>{close();chooseNationalOptional(id)}});
    else buttons.push({l:'Uruchamiam cel',f:()=>{close();startNationalGoal(id)}});
  }
  buttons.push({l:'Zamykam',f:close});
  modal(item.type==='national'?'Cel narodowy':'Cel partyjny',item.name,
    `<p>${g.what||''}</p><div class="note"><b>Warunki:</b><br>${conditions}</div>${effects?`<ul>${effects}</ul>`:''}`,
    buttons,close);
}
